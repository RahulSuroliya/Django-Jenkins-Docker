...
class TimeTableCtl(BaseCtl):
    ...
    def submit(self, request, params={}):
        if (params['id']>0):
            q = self.get_service().get_model.objects.exclude(id=params['id']).filter(subject_ID=self.form['subject_ID'], examTime=self.form['examTime'], examDate=self.form['examDate'])
            print("qqqqqq", q)
            if (q.count()>0):
                self.form['error'] = True
