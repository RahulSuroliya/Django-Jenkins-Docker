...
class ShoppingCartListCtl(BaseCtl):
    count = 1
    ...
    def request_to_form(self, requestForm):
        ...
        self.form['date'] = requestForm.get("date", None)
    ...
    def display(self, request, params={}):
        ShoppingCartListCtl.count = self.form['pageNo']
        record = self.get_service().search(self.form)
        self.page_list = record['data']
        self.form['LastId'] = ShoppingCart.objects.last().id
        res = render(request, self.get_template(), {'pageList':self.page_list, 'form':self.form,'cartList':self.preloadData})
        return res

    def previous(self, request, params={}):
        ShoppingCartListCtl.count -= 1
        self.form['pageNo'] = ShoppingCartListCtl.count
        record = self.get_service().search(self.form)
        self.page_list = record['data']
